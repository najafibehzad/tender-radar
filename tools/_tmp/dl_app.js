...
const state = {
  ...
  filters: { q: '', topic: '', city: '', province: '', kind: '', source: '', days: '', sort: 'newest', hasDeadline: false, today: false },
  view: 'table',
  scanTimer: null,
  lastScanPoll: 0,
};
...
function bind() {
  ...
  $('#btnReset').onclick = () => {
    state.filters = { q: '', topic: '', city: '', province: '', kind: '', source: '', days: '', sort: 'newest', hasDeadline: false, today: false };
    $('#q').value = ''; $('#qClear').classList.remove('on');
    ['#fProvince', '#fCity', '#fKind', '#fSource', '#fDays'].forEach(s => $(s).value = '');
    $('#fSort').value = 'newest';
    $('#fDeadline').checked = false; $('#fToday').checked = false;
